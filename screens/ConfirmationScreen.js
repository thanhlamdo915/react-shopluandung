...
const ConfirmationScreen = () => {
  ...
  const fetchAddresses = async () => {
    try {
      const response = await axios.get(
        `https://backend-shopluandung.onrender.com/addresses/${userId}`
      );
  ...
  const handleEditScreen = (addressId) => {
  ...
  const fetchUsedRewardPoints = async () => {
    try {
      const response = await axios.get(
        `https://backend-shopluandung.onrender.com/rewardPoints/${userId}`
      );
      console.log('Response:', response.data);
      const {rewardPoints, membership} = response.data;
      setUsedRewardPoints(rewardPoints);
      setMembership(membership); // Giả sử `setMembership` là hàm để cập nhật state cho membership trong React
      switch (membership) {
        case 'Silver':
          discountMember = 0.01;
          break;
      ...
      console.error('Error fetching used reward points:', error);
    }
  };
  useEffect(() => {
                ...
                <View style={{marginLeft: 6}}>
                  ...
                  >
                    <Pressable
                      ...
                      onPress={() => handleEditScreen(item._id)}
                    >
                      <Text>Edit</Text>
                    </Pressable>
      ...
      {currentStep === 3 && selectedOption === 'cash' && (
        <View style={{marginHorizontal: 20}}>
          ...
          >
            ...
            >
              <Text style={{fontSize: 16, fontWeight: '500', color: 'gray'}}>
                Discount for membership : ({membership})
              </Text>
            ...
            >
              ...
              >
                {Intl.NumberFormat('it-IT', {
                  style: 'currency',
                  currency: 'VND',
                }).format((total - usedRewardPoints) * (1 - discountMember))}
              </Text>
            </View>
            <View
              style={{
